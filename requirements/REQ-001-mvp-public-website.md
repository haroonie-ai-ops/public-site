...
| A4 | UK GDPR and EU GDPR apply — **re-examined and CONFIRMED 2026-09-18.** Verbatim owner decision: *"'A4 stands' -> zero work."* Worth recording that this was revisited rather than merely never challenged: A4 was adopted 2026-09-10, before any entity information existed, and REQ-001-A3 §5.3 flagged it once the entity became known to be a US (Illinois) LLC — `haroonie.ai LLC`, Springfield, IL. GDPR reaches non-EU controllers who offer services to, or monitor, people in the EU/UK, so the entity's jurisdiction does not by itself settle it; the owner has confirmed the assumption holds. The Privacy Policy's four structured fields are GDPR-framed and consistent with it, so no content change follows. Independently verified the policy's factual claims against the build at the time of this decision: the site sets **no cookies**, loads **no analytics or tracking**, and references **no third-party origin** (CSP is `'self'` throughout); the contact form carries no `action` and does not submit, so the only personal data in scope is what a visitor voluntarily puts in an email. **If R-3.1's form is ever wired (Wave 5, blocked on E4), that changes — personal data would then flow through an actual pipeline, and A4 deserves re-testing at that point rather than inheriting this decision.** |
...
- AC1 — Given a visitor, When `/contact/` loads, Then a contact email address
  ~~and a booking link are~~ **is** visible and ~~both are~~ **is**
  actionable. **Booking clause STRUCK 2026-09-18 by owner decision** —
  verbatim: *"Cancel e22 and p17."* This supersedes the interim waiver
  recorded below: the site is not going to have a booking link, so a clause
  requiring one cannot be satisfied and is removed rather than left
  permanently waived. A waiver is a temporary state with an exit; this has
  no exit, and saying so is more honest than carrying an AC the product will
  never meet. The email half is unchanged and still fully asserted, and a
  published telephone number (E18) now sits alongside it, so the
  requirement's purpose — "enables an enquiry" — is met by two actionable
...
- Note (2026-09-18, superseding two earlier notes on the same day): the
  matrix exists — `status/R8-TRACEABILITY-MATRIX.md`, generated by
  `scripts/traceability.mjs` and drift-checked in CI by `npm run
  trace:check`. **135 acceptance criteria across 48 requirements: 119 carry
  verification that exists and has been re-read (82 asserted by a named test,
  28 by a dated manual record, 9 by CI or platform configuration); 3 are
  blocked on E4; 1 is an accepted limitation; 4 are deviations awaiting an
  owner decision; 7 are struck or inapplicable; 1 is a gap.**
- Note on the two figures this replaces, because they contradicted each other
  and both were wrong. The first said *"9 are gaps"*; the matrix then said
  **GAP: 0**, having closed all nine. QA-006's independent audit (2026-09-18)
  found the zero did not survive scrutiny: at least six criteria were marked
  AUTOMATED with no assertion anywhere, five governed a hero image that does
  not exist, and roughly twenty rows credited a neighbouring criterion's
  evidence. Its diagnosis — *"the matrix was assembled by reading what a test
  was for rather than what it asserts"* — was accepted in full, and every row
  was re-derived from the AC text and the assertions rather than patched.
  The count above is higher than nine because it stops hiding three
  categories the old five verdicts could not express: an accepted limitation
  is not a manual verification, a deviation is not a pass, and a struck
  criterion is not verified.
- AC1 is satisfied in the sense it asks for: every identifier has a verdict,
  and each verdict names what backs it. The gaps, deviations and limitations
  are enumerated in the matrix by name rather than absorbed into a total,
...
| ~~**E17** — production brand source assets must be supplied~~ | **RESOLVED 2026-09-17.** The owner supplied `haroonie-ai-brand-kit.zip`: eight SVGs plus a README. **Mark family accepted as-is** — `haroonie-logo-mark.svg`, `-mark-mono.svg` and `haroonie-app-icon.svg` are pure vector (3-5 paths, no text, no embedded raster) and render correctly from 32px up; the full favicon set was generated from them. **Lockups were NOT production-ready as supplied** and were rebuilt: the wordmark was live `<text>` on a `Manrope -> Montserrat -> Arial` fallback chain, which renders in the wrong face anywhere the font is absent — including `<img>`, CSS `background-image` and favicon contexts, which cannot load external fonts at all — and its `.ai` was pinned to an absolute `x` that only aligned at Manrope's exact metrics, producing a visible gap under any other face. All four lockups were regenerated with the wordmark **outlined to paths** from Manrope (SIL OFL 1.1) at weight 800, verified to contain zero `<text>` elements and zero `font-family` declarations. **Item (e), the three service icons, landed separately on 2026-09-18** and is likewise accepted: 25 SVGs supplied, all on a consistent `0 0 64 64` grid, all well-formed, zero `<text>`, zero embedded rasters, zero `<script>`/`<style>`, zero external references, zero `id` attributes (so inlining several on one page cannot collide), and every colour on-palette. The three required icons share stroke-width 2.2. Two notes, neither blocking: they carry no `currentColor`, so they cannot be recoloured from tokens and would need variants if a dark theme is ever adopted (E22, parked); and each carries a `<title>` naming its service, correct for standalone use but wrong beside a visible heading — handled at render time (see R-9.4 AC1). Only the three approved service icons are adopted; the other 22 were not taken into the repository, since adopting them into a page would be new content requiring its own approval. | resolved |
...
| **E22** (new — decision request) | **Does the site gain a dark theme?** (§5). The sheet's "logo on dark" variant raises the question without answering it. This document recommends light-only for MVP and states the cost of the alternative: contrast surface doubles, CI scan surface doubles, the logo becomes a media-query-dependent asset, and a new invisible-in-one-scheme defect class appears. Adopting a dark theme would be a material requirement change (new ACs, not a restyle). **CLOSED 2026-09-18 — owner decision: no dark theme.** Verbatim: *"Cancel e22 and p17."* R-9.9's light-only policy is therefore the settled position rather than a default awaiting challenge, and the site's `color-scheme: light` declaration is correct rather than provisional. The on-dark logo variant stays in the repository and stays in use exactly where R-9.9 AC3 already permits it — the Open Graph card and app/social icons, which are genuinely dark surfaces — since its existence never implied a site-wide theme. Nothing is deleted and nothing is owed: R-9.1's token layer means a dark theme remains cheap to add if this is ever reopened, it simply is not owed now. | closed — decided, not deferred |
