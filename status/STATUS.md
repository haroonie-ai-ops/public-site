...
| 3 | CI/CD pipeline | **Implemented, independently reviewed (QA-003, pass with findings, zero PRODUCT_DEFECT), PR #1 merged, three successful production deploys since. Not yet formally Accepted** **ACCEPTED (Owner) 2026-09-18** — verbatim: *"I formally accept waves 3/4/6"*. Each had been implemented, independently Tester-reviewed and merged for some time but never formally Accepted, leaving the lifecycle in CLAUDE.md one step short of complete. | E8 (branch-protection plan gate) — owner decision pending |
| 4 | Domain and hosting configuration | **R-7.1–R-7.4, R-7.6 live; R-7.5 (security headers) merged (PR #4, 2026-09-15) as `public/_headers`.** REQ-001-A1 folded into REQ-001 (PR #6), E11/E12/E13 resolved. **New, 2026-09-16 (QA-005 Finding 1): R-7.5 AC2 FAILS on the real production hostname** — Cloudflare's Bot Fight Mode injects a script the static CSP blocks. Fix drafted as REQ-001-A2 (DRAFT, not approved) — see E14/E15 below. This is a revision to already-shipped Wave 4 scope, tracked as Wave 4-R1 in PLAN-001 §7, not a wave restart. **ACCEPTED (Owner) 2026-09-18** — verbatim: *"I formally accept waves 3/4/6"*. Each had been implemented, independently Tester-reviewed and merged for some time but never formally Accepted, leaving the lifecycle in CLAUDE.md one step short of complete. | E14/E15 (owner) block the fix; nothing else in Wave 4 is affected |
| 5 | Enquiry form completion | Not started | Wave 2b (Contact skeleton); blocked on E4 |
| 6 | Performance and cross-browser hardening | **Merged (PR #5, 2026-09-15).** Production measured: Lighthouse Performance 100 all six pages, LCP 856-1204ms, 0 bytes client JS at merge time (`status/PERF-001-wave6-audit.md`). **New, 2026-09-16:** QA-005 found the production hostname now carries ~938 bytes of Cloudflare-injected script (R-5.2 AC2 evidence correction, §3.3 of REQ-001-A2 — AC2 still passes, well inside the 50KB budget) and R-5.3 AC1 (no console errors) currently fails on production for the same CSP reason as R-7.5 AC2. R-7.8 AC4 (in REQ-001-A2, DRAFT) requires R-5.2 AC1's Lighthouse budget to be re-verified once the Function ships, not assumed unaffected. **ACCEPTED (Owner) 2026-09-18** — verbatim: *"I formally accept waves 3/4/6"*. Each had been implemented, independently Tester-reviewed and merged for some time but never formally Accepted, leaving the lifecycle in CLAUDE.md one step short of complete. | E14/E15 (owner) block the re-verification work; the existing PERF-001 baseline stands as recorded until then |
...
| ~~E3~~ | ~~GitHub repo under `haroonie-ai-ops` + secrets configured~~ | **RESOLVED — CLOSED 2026-09-18.** The repo half closed 2026-09-11. The secrets half was left open pending E5's token value and then never revisited, which is exactly the drift this pass exists to catch: it has in fact been proven every deploy since. Evidence, not inference — CI runs #68, #70, #76 and #80 on `main` each executed `Deploy production (Cloudflare Pages)` to success, and that job's own `Check Cloudflare credentials` step passes before it runs. A secret that does not exist cannot deploy a site four times. | Closed 2026-09-18 |
| ~~E5~~ | ~~Cloudflare API token — Account → Cloudflare Pages: Edit (CI only)~~ | **RESOLVED — CLOSED 2026-09-18.** Carried as "owner-actioned 2026-09-11, in progress" for a week after it had demonstrably finished. Same evidence as E3: repeated successful production deployments through this exact token. Its *scope* remains unverifiable by any agent (see E11); its *sufficiency* is settled. | Closed 2026-09-18 |
| E4 | Transactional email credential | Blocks Wave 5 only; not a launch blocker | New |
| ~~E6~~ | ~~Copy: services, bio, legal entity/address, mailbox, booking URL~~ | **RESOLVED — CLOSED 2026-09-18.** All owner-supplied and live: three service areas, the About biography, `haroonie.ai LLC` with its registered Springfield address on Privacy and Terms, and `dev@haroonie.ai` as the enquiry mailbox (plus a published telephone number, which E6 never asked for). The booking URL is the one item never supplied — and no longer wanted: the owner cancelled the booking link outright on 2026-09-18, and R-2.4 AC1's booking clause was struck rather than left waiting. **The placeholder register is now empty**, which was E6's real exit condition. | Closed 2026-09-18 |
| ~~E8~~ | ~~R-6.1 AC2 is unimplementable as specified: branch protection and rulesets are unavailable on private repos on GitHub Free~~ | **RESOLVED — CLOSED 2026-09-18.** Re-verified live rather than taken from the record: the repository is public (`"private": false`) and ruleset **23484592 "main-protection"** is `enforcement=active`. R-6.1 AC2 is satisfiable exactly as originally written; its enforcement clause was never amended away. Already recorded as resolved in REQ-001 on 2026-09-15 but left reading as open here — a divergence between two ledgers describing the same fact. | Closed 2026-09-18 |
| ~~E10~~ | ~~Wave 4's planned Cloudflare access path is proven non-functional~~ | **RESOLVED — CLOSED 2026-09-18.** Overtaken by events and never closed. Wave 4 completed: the `www.haroonie.ai` CNAME was created, the site serves on the custom domain, and the before/after zone captures are published at `status/WAVE4-dns-evidence.md`. A DNS write that demonstrably happened settles whether a working access path was found. | Closed 2026-09-18 |
| E11 | `CLOUDFLARE_API_TOKEN` (CI Pages-deploy secret) scope was never independently verified, and cannot be verified by any agent | **CLOSED AS FAR AS IT CAN BE, 2026-09-18 — and deliberately not marked "resolved".** The original finding stands on its own terms: `GET /user/tokens` and `GET /accounts/{id}/tokens` both return `9109 Unauthorized`, so no agent can enumerate this token's grants, and that has not changed. What *has* been settled is sufficiency rather than scope: the token has deployed this site repeatedly, including a Cloudflare Pages **Function** (Wave 4-R1), which is the broadest thing this program will ask of it — so E16 never triggered. The residual risk is unchanged and worth leaving visible: a token whose grants cannot be read may be **broader** than needed, and nothing here proves it is least-privilege. Closing it as "proven working" would overstate the evidence. | Closed as unverifiable; sufficiency proven, scope still unknown |
...
| ~~E16~~ | ~~Conditional: if deploying the Pages Function requires a Cloudflare grant beyond CI's existing scoped token, stop for owner action~~ | **CLOSED 2026-09-18 — NEVER TRIGGERED.** The condition was tested by events rather than left hypothetical: the Pages Function shipped in Wave 4-R1 and deploys on the existing token, so U19's assumption that Functions need no additional grant held. Closed because the thing it guarded against did not happen, which is a different and weaker claim than "resolved". | Closed 2026-09-18 — not triggered |
| ~~E17~~ | ~~Production brand source assets must be supplied — the actual files, not renders~~ | **RESOLVED — CLOSED 2026-09-18.** All items (a)–(e) delivered and adopted. The mark family and the 25-icon set were accepted after validation; the four wordmark lockups were **rebuilt**, because as supplied their wordmark was live `<text>` on a font fallback chain that renders in the wrong face anywhere Manrope is absent — including `<img>` and favicon contexts, which cannot load external fonts at all. | Closed 2026-09-18 |
| ~~E18~~ | ~~Confirm the three business-card contact details are not to be published as shown~~ | **RESOLVED — CLOSED 2026-09-17.** Owner confirmed all three refusals and supplied a real telephone number. `(312) 555-0100` sits in the NANP reserved fictional range and could not have been published under §1.3 at all; a guard test now fails the build if it ever reappears. | Closed 2026-09-17 |
| E19 | Decide on the two scope items in the header mockup: a "Projects" nav item (a seventh route, needing client work §1.2 places out of scope and §1.3 forbids inventing) and "Get Started" (which both relabels the approved CTA and adds a second header CTA, breaking R-2.1 AC2's "exactly one") | **PARKED by owner 2026-09-17** — verbatim: *"#8 - ignore for now"*. Blocks nothing; the default keeps six routes and one CTA, all currently conforming. | Parked — awaiting owner |
| ~~E20~~ | ~~Brand asset provenance and licensing~~ | **CLOSED 2026-09-18.** Owner confirmed the originals were AI-generated, which resolves the inbound risk this existed for — no stock library, agency or designer holds rights to assert. Trademark clearance closed separately as an **owner-accepted risk**, with what was and was not examined recorded rather than implied. | Closed 2026-09-18 |
| ~~E21~~ | ~~Conditional: if the owner prefers Google Fonts over self-hosting, the CSP widening is its own decision~~ | **CLOSED 2026-09-17 — NEVER TRIGGERED.** Manrope is self-hosted as one 24KB variable woff2; `font-src 'self'` already permitted it, so no CSP directive changed. | Closed — not triggered |
| ~~E22~~ | ~~Does the site gain a dark theme?~~ | **CLOSED 2026-09-18 — owner decision: no.** R-9.9's light-only policy is the settled position rather than a default awaiting challenge. R-9.1's token layer keeps a dark theme cheap if ever reopened; it is simply not owed. | Closed 2026-09-18 |
