...
## Regression Re-verification (Tester, 2026-09-12)

**Result: CONFIRMED. Finding 2's fix holds under direct, live reproduction
of the exact failure mode, independently staged — not re-read from the
Engineer's remediation report.** This closes the "awaiting Tester regression
re-verification" note carried in `status/STATUS.md` since 2026-09-10.

### Method

Ran entirely in `D:\dev\public-site` (`main`, HEAD `b4efc10`), confirmed
Node `24.21.0` active and ports free before starting per this task's
instructions:

1. `git diff 93205ef..b4efc10 -- tests/ playwright.config.ts` — **empty**.
   No test file or `playwright.config.ts` has changed on `main` since the
   QA-002 remediation commit itself. This means every assertion QA-002
   already reviewed byte-for-byte is still exactly what's running today;
   nothing was touched, let alone weakened, in between.
2. Built the site (`npm run build`, exit 0), then started a real,
   long-lived orphan `astro preview --port 4399` process **in the
   foreground, without `--ignore-lock`** — a live stand-in for exactly the
   crashed-prior-run scenario Finding 2 and its remediation describe.
   Confirmed it was genuinely listening (`netstat` showed PID 19436 on
   `4399`, HTTP 200) and that it wrote the lock file
   (`.astro/preview.json`, `pid: 19436, port: 4399`).
3. **Reproduced the pre-fix failure as a control**, with the orphan still
   alive: `npx astro preview --port 4398` (a different, fully free port,
   no `--ignore-lock`) was refused immediately with `"Preview server
   already running at http://127.0.0.1:4399 (pid 19436)"` — confirming
   Finding 2's root cause (the lock blocks by directory, not by port) is
   still real in the installed Astro version, i.e. this is a genuine fix
   to a genuine problem, not a fix to something that no longer reproduces.
4. With the same orphan still alive on `4399`, ran the actual harness
   through the real fix:
   `PW_PORT=4381 PW_PREVIEW_PORT=4382 npx playwright test --project=static-preview`
   → **12/12 passed**. Confirmed immediately after that the orphan's PID
   (19436) and its lock file were byte-for-byte unchanged — the fix did
   not read, touch, or signal the orphan in any way, exactly as designed.
   Killed the orphan and removed the lock myself as cleanup (not part of
   the harness).
5. From a genuinely clean port state (`4381`/`4382` confirmed free, no
   stale `.astro/preview.json`), ran the **full** suite twice in a row:

   - Run 1: **98 passed, 1 skipped, 0 failed**
   - Run 2: **98 passed, 1 skipped, 0 failed**

   Both runs identical. The single skip in both is
   `[webkit] › tests/smoke.spec.ts:85:2 › accessibility scaffolding › a skip
   link is the first focusable element` — the same documented WebKit
   tab-order case carried since Wave 1, not a new or different skip.
   Confirmed clean teardown after both runs: no process left listening on
   either port, no `.astro/preview.json` left behind (expected —
   `--ignore-lock` never writes one).

### Regression check (Wave 1 / Wave 2a assertions)

No Wave 1 or Wave 2a assertion was weakened, removed, or had its coverage
reduced anywhere along the way to this fix. This is not an inference from
the remediation report — step 1 above is a direct diff across the entire
window from QA-002's remediation commit to the current `main` HEAD, and it
is empty. The full-suite counts (98/1/0, twice) exactly match both the
Engineer's remediation-report numbers and QA-002's own prior 98/1/0 finding
for the same suite, confirming nothing regressed since either checkpoint.

### Verdict

**Finding 2: CLOSED, regression-confirmed.** The `astro preview
--ignore-lock` fix in `playwright.config.ts` correctly prevents a crashed
or orphaned prior `astro preview` process in the same working directory
from blocking the `static-preview` webServer, verified by staging that
exact scenario live (not merely re-reading the Engineer's own staged
reproduction) — and it does so without reading, modifying, or signalling
the other process, confirmed by direct before/after inspection of its PID
and lock file. Full suite: **98 passed, 1 skipped, 0 failed**, reproduced
twice from a clean port state. No Wave 1 or Wave 2a assertion was weakened.

Wave 2a's QA-002 status can now be recorded as fully closed, including this
regression re-verification — nothing outstanding remains against it.
