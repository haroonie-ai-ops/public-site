...
| P5 | Privacy (`/privacy/`) | `metaDescription` (`src/content/legal/privacy.md`) | Replaced the Wave 2a stub description with real copy describing the actual policy page (this row covers only the meta description; the *policy body* is P18, closed 2026-09-19) | 2026-09-12, Wave 2b Engineer |
| P6 | Terms (`/terms/`) | `metaDescription` (`src/content/legal/terms.md`) | Replaced the Wave 2a stub description with real copy describing the actual policy page (this row covers only the meta description; the *page body* is P18, closed 2026-09-19) | 2026-09-12, Wave 2b Engineer |
...
| P7 | All pages | `og:image` / `twitter:image` (`src/components/SeoHead.astro`) | **CLOSED.** Replaced `/favicon.svg` with `/og-image.jpg`, a 1200x630 brand social card derived from the owner-supplied brand banner (2026-09-17). The old default was the **unmodified Astro starter logo**, and it was the share image on every page - so shares rendered either nothing (major platforms do not render SVG OG images) or Astro's mark as haroonie.ai's identity. `og:image:width`/`height`/`type`/`alt` added alongside. | 2026-09-17, owner-approved (REQ-001-A3 / decision #3); asset owner-supplied |
| P15 | Home (`/`) | `servicesSummary` | **CLOSED — owner approved the wording 2026-09-17** (decision #11). Agent-drafted because the pre-E6 text described the old positioning and contradicted the three new service areas; derived only from the owner's approved service titles. | 2026-09-17, owner approval |
| P16 | About (`/about/`) | `metaDescription` | **CLOSED — owner approved the wording 2026-09-17** (decision #12). Agent-drafted because the schema requires the field and the owner supplied a heading and body but no meta description; derived strictly from their own approved body copy. | 2026-09-17, owner approval |
| P18 | Privacy (`/privacy/`) + Terms (`/terms/`) | Policy **bodies** (`src/content/legal/privacy.md`, `terms.md`) | **CLOSED — owner read and approved as-is, 2026-09-19.** Verbatim: *"#5 I have read it and approve them as-is."* Both bodies are agent-drafted. They were never marked `placeholder: true` in the schema, because neither invents a REQ-001 section 1.3-banned fact - but they had also never been read by the owner, which is the thing this row existed to track and the reason it was named a go-live blocker rather than a content gap. R-2.5 AC1 was already asserted by `legal.spec.ts` (all four required sections present and non-empty); what was missing was the human judgement that the words are the ones this company wants to stand behind. That judgement is now recorded. The owner declined the offer to route Terms past a lawyer first; that remains open to them at any time and is not a precondition for go-live on this record. | 2026-09-19, owner approval |
| P17 | Contact (`/contact/`) | `bookingUrl`/`bookingLabel` | **CLOSED 2026-09-18 — owner cancelled the booking link outright** (*"Cancel e22 and p17."*). Not resolved by supplying the awaited value: the awaited value is no longer wanted. R-2.4 AC1's booking clause was **struck** from the requirement rather than left permanently waived, the optional schema fields and the conditional render were removed, and the `test.fixme` that had been holding a place for the restored assertion was deleted — a pending test for a cancelled requirement implies work is owed. The active guard against a dead booking link reappearing is **kept**. | 2026-09-18, owner decision |
